...
const JSON_RANKS=[
  ...
  {name:'MASTER',tier:'TIER 4',icon:'\u26A1',desc:'Full sandbox',minLessons:8},
];
const JSON_FEATURES=[
  {label:'View JSON lessons',minRank:0},{label:'Syntax highlighting',minRank:1},{label:'Copy unit JSON',minRank:1},
  {label:'Format JSON',minRank:2},{label:'Basic text editing',minRank:2},{label:'Load any unit JSON',minRank:3},
  {label:'Full editor access',minRank:3},{label:'Import custom units',minRank:4},{label:'Create from scratch',minRank:4},
];
const STORY_FIRED_KEY='__unitforge_story_fired_v1';
const STORY_CHAPTERS=[{rank:1,id:'story_ch1',cs:'story_reader'},{rank:2,id:'story_ch2',cs:'story_student'},{rank:3,id:'story_ch3',cs:'story_coder'},{rank:4,id:'story_ch4',cs:'story_master'}];
const StoryProgress={
...
class UndoStack{constructor(maxDepth=60){this.stack=[];this.idx=-1;this.max=maxDepth;this._skipNext=false;}push(val){if(this._skipNext){this._skipNext=false;return;}if(this.stack[this.idx]===val)return;this.stack.splice(this.idx+1);this.stack.push(val);if(this.stack.length>this.max)this.stack.shift();this.idx=this.stack.length-1;}undo(){if(this.idx>0){this.idx--;this._skipNext=true;return this.stack[this.idx];}return null;}redo(){if(this.idx<this.stack.length-1){this.idx++;this._skipNext=true;return this.stack[this.idx];}return null;}reset(val){this.stack=[val];this.idx=0;}}
const jIdeUndo=new UndoStack();

const JIS={lesson:'intro',code:(()=>{try{return localStorage.getItem(JSON_DRAFT_KEY)||'';}catch{return '';}})(),validation:null,uiInit:false};
...
function jsonLineCount(text){return Math.max(1,String(text||'').split('\n').length);}
function jsonLineNumbers(text){return Array.from({length:jsonLineCount(text)},(_,i)=>i+1).join('\n');}
...
function jsonPreviewHtml(vr){
  ...
  if(!parsed)return `<div style="font-size:9px;color:#3a4a70;line-height:1.75">Valid JSON will show a quick field summary here.</div>`;
...
const JSON_LESSONS=[
  {id:'intro',title:'WHAT IS JSON?',icon:'01',exercise:false,initialCode:'',
    render(){return`<div class="jl-h1">WHAT IS JSON?</div><div class="jl-sub">LESSON 1 / 8 — FOUNDATION</div><div class="jl-p"><strong>JSON</strong> (JavaScript Object Notation) is the text format this game uses to define every unit. It is readable by both humans and machines.</div><div class="jl-p">Mastering JSON gives you <span style="color:#aabbff">direct access to the unit engine</span>.</div><div class="jl-rule"><div class="jl-rule-hdr">THE CORE IDEA</div><div class="jl-rule-body">JSON has two structures: <strong>objects</strong> (named fields in <code>{}</code>) and <strong>arrays</strong> (ordered lists in <code>[]</code>).</div></div><div class="jl-code">${jsonHL('{\n  "name": "PYROS",\n  "school": "fire",\n  "hp": 310,\n  "active": true\n}')}</div>`;},validator:null},
  {id:'objects',title:'OBJECTS & KEYS',icon:'02',exercise:true,initialCode:'{\n  \n}',
    render(){return`<div class="jl-h1">OBJECTS & KEYS</div><div class="jl-sub">LESSON 2 / 8 — OBJECTS</div><div class="jl-p">An <strong>object</strong> is a collection of key-value pairs in <code style="color:#7799ff">{}</code>.</div><div class="jl-code">${jsonHL('{\n  "id":   "shadow_wraith",\n  "name": "WRAITH",\n  "hp":   280\n}')}</div><div class="jl-rule"><div class="jl-rule-hdr">RULES</div><div class="jl-rule-body">• Keys must be in <strong>double quotes</strong>: <code>"key"</code> ✓<br>• Joined by colon: <code>"key": value</code><br>• Pairs separated by commas</div></div><div class="jl-ex"><div class="jl-ex-hdr">⚡ EXERCISE</div><div class="jl-ex-body">Write a valid JSON object with at least <strong>2 key-value pairs</strong>.</div></div>`;},
  ...
  {id:'types',title:'VALUE TYPES',icon:'03',exercise:true,initialCode:'{\n  \n}',
    render(){return`<div class="jl-h1">VALUE TYPES</div><div class="jl-sub">LESSON 3 / 8 — DATA TYPES</div><div class="jl-p">JSON values can be one of 6 types.</div><div class="jl-code">${jsonHL('{\n  "name":   "VORTEX",\n  "hp":     220,\n  "spd":    2.4,\n  "magic":  true,\n  "target": null\n}')}</div><div class="jl-rule"><div class="jl-rule-hdr">THE 6 TYPES</div><div class="jl-rule-body"><span style="color:#44cc88">String</span> — text in double quotes<br><span style="color:#ffaa44">Number</span> — integer or decimal<br><span style="color:#cc77ff">Boolean</span> — <code>true</code> or <code>false</code><br><span style="color:#cc77ff">Null</span> — empty value: <code>null</code><br><span style="color:#7799ff">Object</span> — nested structure<br><span style="color:#7799ff">Array</span> — ordered list</div></div><div class="jl-ex"><div class="jl-ex-hdr">⚡ EXERCISE</div><div class="jl-ex-body">Write a JSON object that contains: a <strong>string</strong>, a <strong>number</strong>, and a <strong>boolean</strong>.</div></div>`;},
    validator(p){if(typeof p!=='object'||Array.isArray(p)||!p)return'Must be a JSON object {}';const vs=Object.values(p);if(!vs.some(v=>typeof v==='string'))return'Missing a string value';if(!vs.some(v=>typeof v==='number'))return'Missing a number value';if(!vs.some(v=>typeof v==='boolean'))return'Missing a boolean';return null;}},
  {id:'arrays',title:'ARRAYS',icon:'04',exercise:true,initialCode:'{\n  \n}',
    render(){return`<div class="jl-h1">ARRAYS</div><div class="jl-sub">LESSON 4 / 8 — ORDERED LISTS</div><div class="jl-p">Arrays store ordered lists with <code style="color:#7799ff">[]</code>.</div><div class="jl-code">${jsonHL('{\n  "schools":  ["fire", "void", "storm"],\n  "passives": ["berserker", "vampiric"],\n  "range":    [20, 200]\n}')}</div><div class="jl-rule"><div class="jl-rule-hdr">ARRAY RULES</div><div class="jl-rule-body">• Square brackets: <code>[]</code><br>• Items separated by commas<br>• Any value type — even mixed</div></div><div class="jl-ex"><div class="jl-ex-hdr">⚡ EXERCISE</div><div class="jl-ex-body">Write a JSON object with at least one array containing <strong>3 or more items</strong>.</div></div>`;},
  ...
  {id:'nested',title:'NESTED OBJECTS',icon:'05',exercise:true,initialCode:'{\n  \n}',
    render(){return`<div class="jl-h1">NESTED OBJECTS</div><div class="jl-sub">LESSON 5 / 8 — DEEP STRUCTURE</div><div class="jl-p">Objects can contain other objects.</div><div class="jl-code">${jsonHL('{\n  "name": "PYROS",\n  "magic": {\n    "school": "fire",\n    "dmg":    45,\n    "cd":     800\n  },\n  "ai": {\n    "stance":     "aggressive",\n    "aggression": 0.75\n  }\n}')}</div><div class="jl-ex"><div class="jl-ex-hdr">⚡ EXERCISE</div><div class="jl-ex-body">Write a JSON object containing at least one <strong>nested object</strong>.</div></div>`;},
  ...
  {id:'schema',title:'UNIT SCHEMA',icon:'06',exercise:false,initialCode:'',
    render(){return`<div class="jl-h1">UNIT SCHEMA</div><div class="jl-sub">LESSON 6 / 8 — COMBAT FORGE FORMAT</div><div class="jl-p">Every unit in this game is a JSON object with this structure.</div><div class="jl-code" style="max-height:300px;overflow-y:auto">${jsonHL('{\n  "id":     "unit_id",\n  "name":   "UNIT NAME",\n  "school": "fire",\n  "hp":     200,\n  "armor":  5,\n  "spd":    2.0,\n  "magic": {\n    "enabled": true,\n    "school":  "fire",\n    "spell":   { "dmg": 35, "cd": 900, "range": 200 }\n  },\n  "melee": { "enabled": false },\n  "ai": {\n    "stance":     "aggressive",\n    "aggression": 0.75\n  }\n}')}</div><div class="jl-rule"><div class="jl-rule-hdr">KEY SECTIONS</div><div class="jl-rule-body"><strong>hp / armor / spd</strong> — base stats<br><strong>magic</strong> — spell system<br><strong>melee / ranged</strong> — attack modes<br><strong>ai</strong> — personality and behavior</div></div>`;},validator:null},
  {id:'modify',title:'MODIFY A UNIT',icon:'07',exercise:true,
    ...
    render(){return`<div class="jl-h1">MODIFY A UNIT</div><div class="jl-sub">LESSON 7 / 8 — HANDS-ON EDIT</div><div class="jl-p">Below is a real unit template in Combat Forge JSON format. Edit it directly.</div><div class="jl-ex"><div class="jl-ex-hdr">⚡ EXERCISE — MODIFY</div><div class="jl-ex-body">1. Change <code>"name"</code> to anything other than <code>"TEMPLATE"</code><br>2. Change <code>"hp"</code> to a different number<br>3. Keep the JSON valid<br>4. Hit <strong>SUBMIT</strong></div></div>`;},
  ...
  {id:'import',title:'BUILD & IMPORT',icon:'08',exercise:true,
    ...
    render(){return`<div class="jl-h1">BUILD & IMPORT</div><div class="jl-sub">LESSON 8 / 8 — MASTER TRIAL</div><div class="jl-p">This is the final lesson. Build a valid unit and press <strong style="color:#44ffaa">IMPORT TO ROSTER</strong>.</div><div class="jl-rule"><div class="jl-rule-hdr">REQUIRED FIELDS</div><div class="jl-rule-body"><code>id</code> — unique string, no spaces<br><code>name</code> — display name<br><code>school</code> — ${Object.keys(SCH).map(s=>`"${s}"`).join('/')}<br><code>hp</code> — number ≥ 50<br><code>spd</code> — number ≥ 0.5</div></div><div class="jl-ex"><div class="jl-ex-hdr">⚡ MASTER TRIAL</div><div class="jl-ex-body">Customize the template or write your own unit. When validation passes, click <strong>IMPORT TO ROSTER</strong>.</div></div>`;},
    validator(p){if(typeof p!=='object'||!p)return'Must be a JSON object';if(!p.id||typeof p.id!=='string')return'Missing "id" (string, no spaces)';if(p.id.includes(' '))return'"id" must not contain spaces';if(!p.name||typeof p.name!=='string')return'Missing "name"';if(!Object.keys(SCH).includes(p.school))return`"school" must be one of: ${Object.keys(SCH).join(', ')}`;if(typeof p.hp!=='number'||p.hp<50)return'"hp" must be a number ≥ 50';if(typeof p.spd!=='number'||p.spd<0.5)return'"spd" must be a number ≥ 0.5';return null;}},
];

function jsonRunValidate(code, lessonId) {
  if (!code || !code.trim()) return { ok: false, error: { message: 'Empty script payload', line: 1, col: 1 } };
  try {
    const parsed = JSON.parse(code);
    const lesson = JSON_LESSONS.find(l => l.id === lessonId);
    if (lesson && lesson.validator) { const err = lesson.validator(parsed); if (err) return { ok: false, error: { message: err, line: 1, col: 1 }, lessonErr: err }; }
...
function renderJsonIde(){
  ...
  const llistHtml=JSON_LESSONS.map(l=>{const done=completed.has(l.id);const active=l.id===JIS.lesson;const icon=done?'\u2713':(active?'\u25B8':l.icon);const col=done?'#44ff88':(active?'var(--acc)':'var(--dim)');return`<div class="json-litem${active?' active':''}${done?' done':''}" onclick="jsonGoLesson('${l.id}')"><span class="json-lnum" style="color:${col}">${icon}</span><span class="json-ltitle">${l.title}</span></div>`;}).join('');
  const featHtml=JSON_FEATURES.map(f=>{const on=rank>=f.minRank;return`<div class="rk-feat ${on?'on':'off'}"><span class="rk-feat-icon">${on?'\u2713':'\u25CB'}</span>${f.label}</div>`;}).join('');
  const toNext=rank<4?Math.max(0,JSON_RANKS[rank+1].minLessons-n):0;
  const sandboxHtml=rank>=4?`<div class="rk-sandbox active">\u26A1 MASTER RANK<br><span style="color:#3a6a50;font-size:7px">All sandbox features active.</span></div>`:`<div class="rk-sandbox">${toNext} more lesson${toNext===1?'':'s'} to unlock <strong>${JSON_RANKS[rank+1].name}</strong>.</div>`;
  const nextChapter=StoryProgress.nextChapter();
  const storyHtml=nextChapter?`<div class="rk-sandbox story">\uD83D\uDCD6 Story beat unlocked at <strong>${JSON_RANKS[nextChapter.rank].name}</strong> rank.</div>`:`<div class="rk-sandbox story active">\uD83D\uDCD6 Story complete.</div>`;
  ...
  if(lesson.exercise){
    ...
    const isDone=completed.has(lesson.id);const isImport=lesson.id==='import';
    const canFmt=JsonFreedom.canFormat();const canLoad=JsonFreedom.canLoadUnit();
    ...
    edSection=`<div class="json-tb"><button class="jbt" onclick="jsonFmt()" ${!canFmt?'disabled':''}>\u229E FORMAT</button><button class="jbt" onclick="jsonRst()">\u21BA RESET</button><button class="jbt" onclick="jsonClr()">\u2715 CLEAR</button>${exportBtn}${loadSel}${subBtn}${impBtn}${doneTag}</div><div style="display:flex;gap:0;border:1px solid #131e38;border-radius:3px;overflow:hidden;background:#010306;min-height:170px;max-height:320px"><pre id="jln" style="margin:0;flex:0 0 38px;overflow-y:auto;overflow-x:hidden;background:#02050b;color:#2a3870;font-family:'Courier New',monospace;font-size:10px;line-height:1.7;padding:10px 6px;text-align:right;user-select:none;white-space:pre">${lineNums}</pre><textarea class="json-ta" id="jta" spellcheck="false" oninput="jsonTaInput(this)" onscroll="jsonSyncLineNumbers(this)" onkeydown="jsonHandleEditorKeys(event,this)" placeholder="// write your JSON here.">${safeCode}</textarea></div><div id="jval" class="json-val ${vr&&vr.ok?'ok':(vr&&vr.error||vr&&vr.lessonErr)?'err':'idle'}">${!String(code||'').trim()?'\u2E61 type JSON below to validate...':vr&&vr.lessonErr?'\u25CB '+vr.lessonErr:vr&&vr.error?'\u2717 Line '+vr.error.line+', Col '+vr.error.col:vr&&vr.ok?'\u2713 Valid JSON — exercise complete!':'\u2E61 type JSON below...'}</div><div style="font-size:8px;color:var(--dim);letter-spacing:1.5px;margin-bottom:4px;margin-top:2px">SYNTAX PREVIEW</div><div style="display:grid;grid-template-columns:minmax(0,1fr) 220px;gap:8px;align-items:start"><div class="json-hl">${code?jsonHL(code):'<span style="color:#1a2540">// output appears here...</span>'}</div><div style="background:#010306;border:1px solid #0a1020;border-radius:3px;padding:9px 10px;font-family:'Courier New',monospace;font-size:9px;line-height:1.7;color:#3a4a70;min-height:120px;overflow:auto">${jsonPreviewHtml(vr)}</div></div>`;
  }else{
...
function jsonGoLesson(id){const lesson=JSON_LESSONS.find(l=>l.id===id);if(!lesson)return;JIS.lesson=id;JIS.code=lesson.initialCode||'';JIS.validation=null;jIdeUndo.reset(JIS.code);jsonSaveDraft(JIS.code);renderJsonIde();}
...
function jsonTaInput(ta){jIdeUndo.push(ta.value);JIS.code=ta.value;jsonSaveDraft(ta.value);const vr=jsonRunValidate(ta.value,JIS.lesson);JIS.validation=vr;
  const ln=document.getElementById('jln');if(ln)ln.textContent=jsonLineNumbers(ta.value);
  const jhl=document.querySelector('.json-hl');if(jhl)jhl.innerHTML=ta.value?jsonHL(ta.value):'<span style="color:#1a2540">// output appears here...</span>';
  const v=document.getElementById('jval');if(v){v.className='json-val '+(vr&&vr.ok?'ok':(vr&&(vr.error||vr.lessonErr))?'err':'idle');v.textContent=!String(ta.value||'').trim()?'\u2E61 type JSON below to validate...':vr&&vr.lessonErr?'\u25CB '+vr.lessonErr:vr&&vr.error?'\u2717 Line '+vr.error.line+', Col '+vr.error.col+': '+vr.error.message:vr&&vr.ok?'\u2713 Valid JSON — exercise complete!':'\u2E61 type JSON below...';}
}
function jsonSyncLineNumbers(ta){const ln=document.getElementById('jln');if(ln)ln.scrollTop=ta.scrollTop;}
function jsonHandleEditorKeys(e,ta){
  const start=ta.selectionStart,end=ta.selectionEnd,value=ta.value;
  if(e.key==='Tab'){e.preventDefault();if(!e.shiftKey){ta.value=value.substring(0,start)+'  '+value.substring(end);ta.selectionStart=ta.selectionEnd=start+2;}else{const lineStart=value.lastIndexOf('\n',start-1)+1;if(value.substring(lineStart,lineStart+2)==='  '){ta.value=value.substring(0,lineStart)+value.substring(lineStart+2);ta.selectionStart=ta.selectionEnd=Math.max(lineStart,start-2);}}jsonTaInput(ta);return;}
  ...
  const pairs={'{':'}','[':']','"':'"'};if(pairs[e.key]!==undefined){e.preventDefault();ta.value=value.substring(0,start)+e.key+pairs[e.key]+value.substring(end);ta.selectionStart=ta.selectionEnd=start+1;jsonTaInput(ta);}
}
...
function jsonRst(){const lesson=JSON_LESSONS.find(l=>l.id===JIS.lesson);if(!lesson)return;const ta=document.getElementById('jta');if(!ta)return;ta.value=lesson.initialCode||'';jIdeUndo.reset(ta.value);jsonTaInput(ta);}
...
function jsonSubmit(){const vr=JIS.validation||jsonRunValidate(JIS.code,JIS.lesson);if(!vr||!vr.ok)return;const currentId=JIS.lesson;JsonFreedom.complete(currentId);showJIdToast(currentId);checkStoryProgress();const idx=JSON_LESSONS.findIndex(l=>l.id===currentId);const next=JSON_LESSONS[idx+1];setTimeout(()=>{if(next){JIS.lesson=next.id;JIS.code=next.initialCode||'';}JIS.validation=null;jsonSaveDraft(JIS.code);renderJsonIde();},1200);}
